...
String catchStreamedExceptions(http.StreamedResponse? response, error) {
  String? msg;
  if (error is Exception) {
    try {
      if (error is HttpException) {
        switch (response?.statusCode) {
          ...
          case 404:
            ...
            break;
          case 405:
            msg = MethodNotAllowed(response?.reasonPhrase).toString();
            break;
          case 406:
            msg = NotAcceptable(response?.reasonPhrase).toString();
            break;
          case 407:
            msg = ProxyAuthenticationRequired(response?.reasonPhrase).toString();
            break;
          case 408:
            msg = RequestTimeOut().toString();
            break;
          case 409:
            msg = Conflict(response?.reasonPhrase).toString();
            break;
          case 410:
            msg = Gone(response?.reasonPhrase).toString();
            break;
          case 413:
            msg = PayloadTooLarge(response?.reasonPhrase).toString();
            break;
          case 429:
            msg = TooManyRequests(response?.reasonPhrase).toString();
            break;
          case 500:
            msg = InternalServerError(response?.reasonPhrase).toString();
            break;
          case 501:
            msg = NotImplemented(response?.reasonPhrase).toString();
            break;
          case 502:
            msg = BadGateway(response?.reasonPhrase).toString();
            break;
          case 503:
            msg = ServiceUnavailable(response?.reasonPhrase).toString();
            break;
          case 504:
            msg = GatewayTimeOut(response?.reasonPhrase).toString();
            break;
          case 505:
            msg = HttpVersionNotSupported(response?.reasonPhrase).toString();
            break;
          case 506:
            msg = VariantsAlsoNegotiates(response?.reasonPhrase).toString();
            break;
          case 507:
            msg = InsufficientStorage(response?.reasonPhrase).toString();
            break;
          case 508:
            msg = LoopDetected(response?.reasonPhrase).toString();
            break;

          case 510:
            msg = NotExtended(response?.reasonPhrase).toString();
            break;
          case 511:
            msg = NetworkAuthRequired(response?.reasonPhrase).toString();
            break;
        }
      } else if (error is SocketException) {
        msg = NoInternetConnexion().toString();
      ...
      return FormatException(response?.reasonPhrase).toString();
    } on PlatformException catch (_) {
      return UnexpectedError().toString();
    } catch (_) {
      msg = UnexpectedError().toString();
